...
pub fn init_individual_discrete(rng: &mut ThreadRng, problem: &TicketProblem, customers: &Vec<Customer>, n_resample:u32, tau:f32, step_size:i32) -> Individual {
    
    let max_wtp = customers.iter().map(|c| c.wtp).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();

    let n_steps = (max_wtp*tau) as i32 / step_size;
